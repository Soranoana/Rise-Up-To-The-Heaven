...
public class playerControll : MonoBehaviour {
    ...
    private void ItemGet(Collider other,int ItemNumber) {
        ...
        if (tmp < 0) return;//持てません処理未実装
        else other.gameObject.GetComponent<dropItem>().settingDestroySelf(0);
    ...
    private void itemSetOnStart() {
        ...
        for (int i = 0; i<itemKindNum; i++) {
            ...
            //Debug.Log(text[0]);
            InventorySet[i]=new ItemAndWeaponSet() { ItemNumber=int.Parse(text[0]),
                ItemName=text[1],
